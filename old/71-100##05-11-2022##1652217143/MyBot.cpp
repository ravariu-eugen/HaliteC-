...
private:
    ...
    hlt::Location findEnemy(hlt::Location loc){
        ...
        for(int i = 1; i <= SIGHT_RADIUS; i++){
            for(int j = 0; j < i; j++){
                int dx = i-j;
                hlt::Location nloc = {loc.x+j,loc.y-dx};//1
                if(inBounds(nloc)){
                    if(isEnemy(nloc))
                        return nloc;
                }
                nloc = {loc.x+dx,loc.y+j};//2
                if(inBounds(nloc)){
                    if(isEnemy(nloc))
                        return nloc;
                }
                nloc = {loc.x-j,loc.y+dx};//3
                if(inBounds(nloc)){
                    if(isEnemy(nloc))
                        return nloc;
                }
                nloc = {loc.x-dx,loc.y-j};//4
                if(inBounds(nloc)){
                    if(isEnemy(nloc))
                        return nloc;
                }
...
public:
    ...
    hlt::Move assignMove(hlt::Location loc){
        ...
        if(site.strength < 5*site.production)
            return {loc,STILL};
        uint8_t bestDir;
        hlt::Location enemyLocation = findEnemy(loc);
        if(enemyLocation.x != 1<<15){
            bestDir = getDirectionTo(loc, enemyLocation);
        }
        else
        if(onBorder(loc)){
