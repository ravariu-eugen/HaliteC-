...
private:
    ...
    int tileValue(hlt::Location loc){
        ...
        //if(!isContested(loc))
            return 100.0*site.production*site.production/(1+ site.strength);
        //else  {
          //  return 10.0*site.production*(site.strength+1);
        //}
    }
    ...
    int moveValue(hlt::Move move){
        ...
        if(neighborSite.owner == 0){
            ...
            return 100 * std::min(0, MAX_STRENGTH - (currentSite.strength + neighborSite.strength)) + locPotential(move.loc, move.dir);
    ...
    void initPotential(){
        ...
        /*std::string filename = "potential" + std::to_string(turnCount) + ".txt";
        std::ofstream ofPot(filename);
        for(unsigned short y = 0; y < gameMap.height; y++) {
            ofPot<<std::setw(3)<< y<<") ";
            for(unsigned short x = 0; x < gameMap.width; x++)
            {
                ofPot<<std::setw(7)<< locOwner({x,y}) << " ";
        ...
        for(unsigned short y = 0; y < gameMap.height; y++) {
            ...
            for(unsigned short x = 0; x < gameMap.width; x++)
            {
                ofPot<<std::setw(7)<< tileValue({x,y}) << " ";
        ...
        for(unsigned short y = 0; y < gameMap.height; y++) {
            ...
            for(unsigned short x = 0; x < gameMap.width; x++)
            {
                ofPot<<std::setw(7)<<std::setprecision(6)<< potential[x][y] << " ";
...
public:
    ...
    hlt::Move assignMove(hlt::Location loc){
        ...
        if(site.strength < std::max(5*site.production,10))
            return {loc,STILL};
        if(onBorder(loc)){
            ...
            if(locStren(loc, chosenMove.dir) >= site.strength && locOwner(loc, chosenMove.dir) != myID)
                return{loc,STILL};
        ...
        uint8_t bestDir = findBorder(loc);
